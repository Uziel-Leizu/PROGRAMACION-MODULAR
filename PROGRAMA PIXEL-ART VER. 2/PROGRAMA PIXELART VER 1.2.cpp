...
int main (void){
    ...
    while(res == 's'){
    system("cls");
    gotoxy(10,1);cout<<"BIENVENIDO AL PROGRAMA PARA CREAR PIXELARTS VER. 1.2"<<endl<<endl;
    menu();
...
void crear (void){
    ...
    salida.open("PIXELARTS/pix.dat",ios::app | ios::binary);
...
void visualizar (void){
	...
	ofstream salida;
    ifstream entrada;
   entrada.open("PIXELARTS/pix.dat",ios::binary);
   while(entrada.read((char*)&pix,sizeof(tamanio))){
   	system("cls");
   	cout<<"PIXELART "<<i+1<<endl<<endl;
for(int f=0;f<=pix.y+2;f++){
		cout<<endl;
        for(int c=0;c<=pix.x+2;c++){
        gotoxy(c+10,f+3);cout<<pix.p [f][c]<<" ";
...
void editar (void){
	...
	int op=1;
	ofstream salida;
	ensa.open("PIXELARTS/pix.dat", ios::in | ios::binary);
int noRegistro = 0;
cout << "Ingrese el numero de pixelart que desea editar: " << endl;
cin >> noRegistro;
 cin.ignore(256,'\n');
system("cls");
ensa.seekg( (noRegistro-1) * sizeof(tamanio), ios::beg );
...
while (op != 5){
    ...
    cin>>res;
			...
			case 4:{
				...
				salida.open("PIXELARTS/pix.dat",ios::in | ios::binary);
				salida.seekp( (noRegistro-1) * sizeof(tamanio), ios::beg );
				salida.write((char*)&pix,sizeof(tamanio));
				salida.close();
